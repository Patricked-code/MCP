...
export function buildGitPullCommand(project: ProjectKey): string {
  ...
  if (project === 'stablecoin_frontend') {
    return `set -euo pipefail
cd ${shellQuote(config.path)}
printf 'Projet: ${config.label}\\nChemin: ${config.path}\\n\\n'
test -d .git
...
test "$CURRENT_BRANCH" = "main" || {
  echo "ERREUR: Stablecoin doit rester sur main."
  exit 21
}

DIRTY="$(git status --porcelain=v1 --untracked-files=all | grep -v '^?? tmp/restart.txt
cd ${shellQuote(config.path)}
mkdir -p .mcp_logs
...
node ${shellQuote(script)} ${quotedArgs}`;
 ...
 || true)"
test -z "$DIRTY" || {
  echo "ERREUR: working tree Stablecoin non propre."
  printf '%s\\n' "$DIRTY"
  exit 22
}
...
case "$REMOTE_URL" in
  https://github.com/Patricked-code/Stablecoin.git|git@github.com:Patricked-code/Stablecoin.git)
    ;;
  *)
    echo "ERREUR: remote github absent ou inattendu."
    exit 23
    ;;
esac

echo "Remote github vérifié."
printf 'HEAD avant: '; git rev-parse HEAD
REMOTE_SHA="$(git ls-remote github refs/heads/main | awk '{print $1}' | head -1)"
test -n "$REMOTE_SHA" || {
  echo "ERREUR: SHA distant main introuvable."
  exit 24
}
...
test "$FETCHED_SHA" = "$REMOTE_SHA" || {
  echo "ERREUR: incohérence entre ls-remote et github/main après fetch."
  exit 25
}
...
if [ "$LOCAL_SHA" = "$REMOTE_SHA" ]; then
  ...
  git merge-base --is-ancestor "$LOCAL_SHA" "$REMOTE_SHA" || {
    echo "ERREUR: serveur ahead/divergé; fast-forward refusé."
    exit 26
  }
