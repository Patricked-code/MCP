...
test('GitHub MCP secret safety detects token and key content', () => {
  ...
  assert.equal(detectSecretLikeContent('token github_pat_1234567890abcdef').code, 'github_fine_grained_token_signal_detected');
  assert.equal(detectSecretLikeContent('-----BEGIN OPENSSH PRIVATE KEY-----').code, 'private_key_block_detected');
  assert.equal(detectSecretLikeContent('TOKEN=abc123456789').code, 'env_secret_assignment_detected');
