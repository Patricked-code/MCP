...
- Live State `228` atteste GitHub/S1/origin-main/runtime au SHA exact du merge, S1 propre/read-only et runtime healthy. Les deux gaps restants au moment de cette écriture sont la projection `deploymentExactShaSuccess` (champ `runtimeRevision` de la tâche encore `null`) et le présent `DOCUMENTATION_DRIFT`.
